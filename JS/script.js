...
function testarLogin() {
  ...
  if (!arroba || !ponto) {
    ...
    if (tamSenha < 6) {
      alert(
        "Senha Inválida, nossas senhas precisam ter pelo menos 6 caracteres"
      );
    } else {
      if (senha != senhaPiloto || email != userPiloto) {
        ...
        var ret = window.confirm(
          "Olá " + email + "! deseja logar com esta conta?"
        );
        if (ret) {
...
function obterVideoIdPorExercicio(exercicio) {
  var videoIds = {
    ...
    "Rosca bíceps martelo no banco inclinado":
      "KnymofpyiIQ?si=qksvNDZZ83yVXS41",
...
function exibirExerciciosInferiores(membro) {
  var exerciciosContainer = document.getElementById(
    "exercicios-container-inferiores"
  );
...
function exibirAbdominais() {
  ...
  var exercicios = obterExerciciosPorMembroInferiores(
    ("Reto", "Infra", "Bicicleta", "Russian twist")
  );
  exercicios.forEach(function (exercicio) {
    var listItem = document.createElement("li");
    listItem.textContent = exercicio;
  ...
  for (var i = 0; i < membros.length; i++) {
    if (
      membros[i].textContent.toLowerCase() ===
      ("Reto", "Infra", "Bicicleta", "Russian twist").toLowerCase()
    ) {
      membros[i].classList.add("active");
      break;
    }
...
function calcImc() {
  ...
  document.getElementById("resultImc").innerHTML = IMC.toPrecision(2);

  if (IMC >= 18.49 && IMC <= 25) alert("Seu IMC está dentro do nível saudável!");
  else if (IMC > 25) alert("Seu IMC está acima do nível saudável!");
  else alert("Seu IMC está abaixo do nível saudável!");
}
